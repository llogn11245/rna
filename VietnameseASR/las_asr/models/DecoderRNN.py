...
class DecoderRNN(nn.Module):
    def __init__(self, vocab_size, max_len, hidden_size, encoder_size,
                 ...
                 dropout_p=0, use_attention=True, device = 'cuda'):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, context, attn_w, function):
        ...
        for i in range(embedded.size(1)):
            ...
            pred = torch.log(self.softmax(self.fc(output)))
            # if use_teacher_forcing:
            #     next_character_p, characters = self.get_next_p(next_character_p=next_character_p, pred = input_var[:,i], characters=characters)
            
            # pred_with_language_model = pred + next_character_p
            y_all.append(pred)
            # if not use_teacher_forcing:
            #     next_character_p, characters = self.get_next_p(next_character_p=next_character_p, pred = pred_with_language_model, characters=characters)
    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None,
        ...
        # next_character_p = self.lm["18:18:18"]
        # next_character_p = next_character_p.repeat(batch_size, 1).to(self.device)
        # characters = ["18:18:18"]*batch_size
        #next_character_p = None
        #next_character_p = None
        #characters = None
    ...
    def get_next_p(self, next_character_p, pred, characters):
        if len(pred.shape) >= 2:
            new_chars = pred.argmax(dim = -1)
        else:
            new_chars = pred
        for index in range(0, next_character_p.shape[0]):
            c1 = characters[index].split(':')[1]
            c2 = characters[index].split(':')[2]
            characters[index] = f"{c1}:{c2}:{new_chars[index]}"
            next_character_p[index] = torch.tensor(self.lm[characters[index]])
        return next_character_p, characters
