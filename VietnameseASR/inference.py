...
def parse_characters(path, text):
    ...
    encoded_text = [word2index.get(x, word2index['unk']) for x in t.split(' ')]
...
if __name__ == "__main__":
    ...
    asr_brain.hparams.cer_file = os.path.join(hparams["output_dir"], "cer_valid.txt")
    asr_brain.hparams.wer_file = os.path.join(hparams["output_dir"], "wer_valid.txt")
    asr_brain.evaluate(
        valid_dataset,
        max_key="ACC",
    ...
    asr_brain.hparams.cer_file = os.path.join(hparams["output_dir"], "cer_test.txt")
    asr_brain.hparams.wer_file = os.path.join(hparams["output_dir"], "wer_test.txt")
    asr_brain.evaluate(
        test_dataset,
        max_key="ACC",
        test_loader_kwargs=hparams["test_dataloader_opts"],
